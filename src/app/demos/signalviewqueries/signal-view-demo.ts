...
export class SignalViewDemoComponent {
  ...
  employees =
    viewChildren<ElementRef<HTMLInputElement>>('employee');
  ...
  showEmployee() {
    ...
    const name =
      this.employeeName()?.nativeElement.value.trim() ?? '';
  ...
  showEmail() {
    ...
    const email =
      this.employeeEmail().nativeElement.value.trim();
  ...
  showEmployees() {
    ...
    const employeeNames = this.employees()
      .map((employee) =>
        employee.nativeElement.value.trim()
      )
      .filter((name) => name !== '');
